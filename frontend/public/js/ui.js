...
export function renderStatus(containerId, status, error) {
    const el = document.getElementById(containerId);
    if (!el)
        return;
    if (status === 'loading')
        el.innerHTML = '<p class="status-loading">Завантаження...</p>';
    else if (status === 'empty')
        el.innerHTML = '<p class="status-empty">Немає даних</p>';
    else if (status === 'error')
        el.innerHTML = `<p class="status-error">Помилка: ${error ?? 'невідома'}</p>`;
    else
        el.innerHTML = '';
}
...
export function renderUsersToSelect(users) {
...
export function renderPostsTable(posts, users) {
...
export function renderPostModal(post, users, comments) {
    const user = users.find(u => u.id === post.userId);
