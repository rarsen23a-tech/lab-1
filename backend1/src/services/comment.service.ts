...
export const getAll = async (query?: GetAllQuery) => {
    let comments = await commentRepository.findAll();
    ...
    return { items, total, page, limit: pageSize, totalPages, hasNext };
};

export const getById = async (id: number | string): Promise<Comment> => {
...
export const create = async (data: CreateCommentDto): Promise<Comment> => {
    ...
    return await commentRepository.create({ postId, userId, text });
};

export const update = async (id: number | string, data: UpdateCommentDto): Promise<Comment> => {
...
export const remove = async (id: number | string) => {
    ...
    const success = await commentRepository.remove(commentId);
    if (!success) {
        throw new AppError(404, 'Comment not found');
    }
